...
public:
        ...
        {
            {
                TT::CURLY_BRACKET_CLOSE, "*", [this](const LexerToken& token)
                {
                    ...
                    if (!currentInterface_.async_uuid.empty())
                    {
                        Interface asynInterface = Interface{};
                        asynInterface.name = std::string("Async") + currentInterface_.name;
                        asynInterface.uuid = currentInterface_.async_uuid;
                        asynInterface.sync_uuid = currentInterface_.uuid;
                        asynInterface.async = true;
                        asynInterface.disp = false;
                        asynInterface.dual = false;
                        asynInterface.ole = false;
                        asynInterface.typelib = false;
                        interfaces_.push_back(asynInterface);
                    }
    ...
    void print()
    {
        for (auto& i : interfaces_)
        {
            ...
            if (!i.async_uuid.empty())
                std::cout << "[async_uuid: " << i.async_uuid << "] ";

            if (!i.sync_uuid.empty())
            {
                std::cout << "[sync_uuid: " << i.sync_uuid << "} ";
            }
        ...
        {
            std::cout << std::endl;
            std::cout << "coclass: " << c.name << " " << c.uuid << " ";
            if (c.control)
                std::cout << "control";
            std::cout << std::endl;
